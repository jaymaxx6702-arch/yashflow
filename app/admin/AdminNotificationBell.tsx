...
export default function AdminNotificationBell({ employeeId }: Props) {
  ...
  const [preferenceKeys, setPreferenceKeys] = useState<
  ...
  const notificationAllowed = useCallback(
    (item: Pick<NotificationRow, "notification_type" | "related_type">) =>
      preferenceKeys.includes(
        notificationPreferenceKey(
  ...
  const loadNotificationPreferences = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data?.error || "Notification preferences load failed."
        );
      }
      ...
      console.warn("Admin notification preferences load failed:", error);
    } finally {
      setPreferencesReady(true);
    }
  }, []);
  ...
  const saveNotificationPreferences = useCallback(
    async (next: AdminNotificationPreferenceKey[]) => {
      setPreferencesBusy(true);
      setPreferenceKeys(next);
      setMessage("");
      ...
      if (!session?.access_token) {
        ...
        setMessage("Admin session મળ્યો નથી.");
        return;
      }
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            data?.error || "Notification preferences save failed."
          );
        }

        setPreferenceKeys(
          Array.isArray(data.selected) ? data.selected : next
        );
        setMessage("Notification selection saved ✅");
      } catch (error) {
        setMessage(
          error instanceof Error
            ? `Preference Save Error: ${error.message}`
  ...
  function toggleNotificationPreference(
    ...
    const next = preferenceKeys.includes(key)
      ? preferenceKeys.filter((item) => item !== key)
      : [...preferenceKeys, key];
  ...
  const loadNotifications = useCallback(
    ...
    [employeeId, notificationAllowed, runAlert]
  );

  useEffect(() => {
    if (!employeeId || !preferencesReady) return;
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (
            newId &&
            !knownIdsRef.current.has(newId) &&
            notificationAllowed({
              notification_type:
                newRow.notification_type || null,
              related_type: newRow.related_type || null,
            })
          ) {
          ...
          <aside className="absolute right-0 top-0 h-full w-full sm:w-[440px] bg-slate-50 shadow-2xl flex flex-col">
            ...
            <div className="mx-4 mt-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs font-semibold text-slate-500 mt-1">
                    Adminને ફક્ત ON રાખેલી categoryના alerts મળશે.
                  </p>
                </div>
                ...
                <button
                  ...
                  onClick={() =>
                    void saveNotificationPreferences(
                      ADMIN_NOTIFICATION_PREFERENCES.map(
                        (item) => item.key
                      )
                    )
                  }
                  className="text-xs font-black text-blue-700 disabled:opacity-50"
                >
                  All ON
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-3">
                {ADMIN_NOTIFICATION_PREFERENCES.map((item) => {
                  const enabled = preferenceKeys.includes(item.key);
                  ...
                  return (
                    <button
                      ...
                      disabled={preferencesBusy}
                      onClick={() =>
                        toggleNotificationPreference(item.key)
                      }
                    ...
                    >
                      ...
                      <p className="text-[10px] font-semibold mt-1 leading-4 opacity-80">
                        {item.description}
                      </p>
                    </button>
                  );
